...
//
// What it checks:
//   1. Every Tier-A wallet (scoreV2 ≥ 50) should pass the MM classifier.
...
//   3. Spearman correlation between scoreV2 and decidedROI across the full
//      pool — should be positive (target ≥ 0.5). Running baseline.
//   4. Evict-rank sanity: bottom 10% by scoreV2 should have low/negative
...
const ranked = allWallets
  .filter(w => typeof w.scoreV2 === 'number')
  .sort((a, b) => b.scoreV2 - a.scoreV2);

if (ranked.length === 0) {
  console.error(`No wallets in pool have scoreV2 populated. Cannot validate.`);
  process.exit(1);
}

// ── Spearman correlation (scoreV2, decidedROI) ──────────────────────────────
...
// ── Checks ──────────────────────────────────────────────────────────────────
const tierA = ranked.filter(w => (w.scoreV2 || 0) >= TIER_A_CUTOFF);
...
for (const w of ranked) {
  if (w.stats?.decidedROI != null) {
    scoreV2s.push(w.scoreV2);
    decidedROIs.push(w.stats.decidedROI);
  }
}
const corr = spearman(scoreV2s, decidedROIs);
...
if (JSON_OUT) {
  ...
  if (c1.items.length > 0) {
    for (const x of c1.items.slice(0, 5)) {
      console.log(`       ${x.address.slice(0, 12)}…  scoreV2=${x.scoreV2}  mm=${x.mmScore}/6  ${x.triggered.join(', ')}`);
    }
  ...
  if (c2.items.length > 0) {
    for (const x of c2.items.slice(0, 5)) {
      console.log(`       ${x.address.slice(0, 12)}…  scoreV2=${x.scoreV2}  edgePP=${x.edgePP}  ${x.reason}`);
    }
    if (c2.items.length > 5) console.log(`       … and ${c2.items.length - 5} more`);
  }

  const c3 = report.checks.spearmanScoreV2VsDecidedROI;
  console.log(`\n  ${icon(c3.severity)}  Spearman(scoreV2, decidedROI): ${c3.value != null ? c3.value.toFixed(3) : 'n/a'} (target ≥ ${c3.target}, n=${c3.sampleSize})`);
